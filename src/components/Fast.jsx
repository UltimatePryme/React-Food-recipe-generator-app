...
function Fast() {
        ...
        const getFast = async() => {
            const api = await fetch(`https://api.spoonacular.com/recipes/random?apiKey=c422330075c64e6bb8e232b82312270a&number=24&cheap=true`);
            const apiData = await api.json();
                setFastList(apiData.recipes)
  ...
  return (
    <main className='bg-my-lightgreen grid grid-cols-1 justify-center'>
        <section className=''>
            <div  className=' bg-gradient-to-t from-my-green  w-full h-[26rem] relative'>
                ...
                <div className='flex items-center justify-center'>
                    <h1 className='absolute mb-16 bottom-5 left-0 right-0 z-2 font-Inter font-black md:text-4xl lg:text-4xl xl:text-6xl text-center text-my-lightgreen'>Affordable meals <br/> every one can make at home</h1> 
                </div>
            </div>
        </section>
        <section  className='px-32 pb-10 w-full h-3/6 grid sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3 gap-1 z-0 justify-center'>
            {FastList.map((recipe) => {
                if (recipe && recipe.readyInMinutes <= 45){
                    return(                    
